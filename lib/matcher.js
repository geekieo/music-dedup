...
// 与旧 subdivideDurBucket 同规则：超限时长桶按指纹段键细分至多 3 层，只把最终
// 会参与比对的配对收集进 pairSet。skip=simMap 用于频谱侧排除 5a 已确认的精确对
//（CP 侧无预置对，不传）。纯内存 Set 操作，收集比相似度计算快几个量级，串行即可。
...
// 分片哈希：把配对均匀散到并发池各分片。同一个配对无论从 LSH 还是时长桶来都落
// 同一分片（pairKey 一致），跨子阶段去重/合并天然成立。
...
export async function runSmartKeep(db, opts = {}) {
  ...
  if (!groups.length) {
    prog.emit({ phase: 'smartKeep', pct: 100, level: 'info', message: '开始智能保留（无未处理组）...' });
    prog.done('没有未处理的重复组');
    return;
  }
  prog.emit({ phase: 'smartKeep', pct: 5, level: 'info', message: `开始智能保留（${groups.length} 个未处理组）...` });
...
export async function runFpMatcher(db, opts = {}) {
  ...
  // ── 相似度计算：并发池分片 或 串行兜底 ──
  // 并发时按 pairKey 哈希分片到 matchPool（并发=设置页「同时处理文件数」），频谱与
  // CP 配对同池交错、两套算法并行；分片数上限=池并发，配对少时收敛到 1 片。
  const results = { spectral: new Map(), cp: new Map() };
  if (matchPool) {
    // 指纹记录一次性注入所有 worker（结构化克隆；worker 只算相似度，不碰库，
    // 无锁竞争）。只传 id/指纹两列，省去 path/title 等无关字段。
    ...
    const settled = await Promise.allSettled(jobs.map((sh) =>
      matchPool.computeShard({ maxOffset: FP_MAX_OFFSET, spectralPairs: sh.spectral, cpPairs: sh.cp }).then((r) => {
        ...
        prog.emit({ phase: 'fpMatch', pct: 60 + Math.round(done / jobsTotal * 30), subPct: Math.round(done / jobsTotal * 100), level: 'ok',
          message: `声纹比对: ${done} / ${jobsTotal} 分片完成` });
  ...
  // ── 应用并查集：阈值 + 佐证门（与旧 comparePair/compareCpGroup 同规则）──
  for (const key of spectralPairs) {
