...
export async function runSmartKeep(db, opts = {}) {
  ...
  if (!groups.length) { prog.done('没有未处理的重复组'); return; }
  prog.emit({ phase: 'smartKeep', pct: 5, level: 'info', message: `重新计算 ${groups.length} 个未处理组的智能保留...` });
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    updated++;
    if (i % 50 === 0 || i === groups.length - 1) {
      prog.emit({ phase: 'smartKeep', pct: 5 + Math.round((i + 1) / groups.length * 90), level: 'ok', message: `智能保留: ${i + 1} / ${groups.length} 组` });
      await prog.flush();
    }
  }
  await onPause();
  prog.done(`智能保留完成：更新 ${updated} 个组`);
}
...
export async function runFpMatcher(db, opts = {}) {
  ...
  // 进度分母只数参与比较的多文件桶：单文件桶 O(1) 跳过
  const prefixEntries = [...prefixMap.values()];
  const prefixTotal = prefixEntries.filter(g => g.length >= 2).length;
  let prefixGroups = 0;
  for (const group of prefixEntries) {
    if (group.length < 2) continue;
  ...
  if (cpFiles.length >= 2) {
    ...
    const cpPrefixEntries = [...cpPrefixMap.values()];
    const cpPrefixTotal = cpPrefixEntries.filter(g => g.length >= 2).length;
    for (const group of cpPrefixEntries) {
      if (group.length < 2) continue;
