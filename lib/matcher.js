...
export async function runMatcher(db, opts = {}) {
  ...
  // >= threshold, with NO independent corroboration — fine for Phase 1
  // (identical fingerprint, not a coincidence), but Phase 3/4 run over much
  // larger candidate sets (duration buckets now allow up to 600 files —
  // see阶段4's cap). Union-Find is transitive: even a small per-comparison
  // false-positive rate, multiplied by the O(n²) comparisons in a large
  // bucket, only needs to produce ONE spurious edge between two otherwise-
  // unrelated songs to chain-merge dozens of completely different tracks
  // into one nonsensical "duplicate group" (this is what was behind
  // multiple unrelated pop songs — Last Kiss, Bang Bang, Stressed Out, etc.
  // — all landing in one group). Below ~97% (essentially certain to be the
  // same audio regardless of tags), now also require SOME independent
  // corroboration — title or artist text actually overlapping — before
  // trusting a bare spectral/Chromaprint score enough to union.
  ...
  // recording id VIA A PRECISE MATCH (match_basis='exact' — corroborated by
  // album/duration/track, not just a title-only guess), that is strong
  // third-party confirmation they're the same underlying recording —
  // regardless of how different their local tags, filenames, or fingerprints
  // look. A 'fuzzy' (title-only) scrape is explicitly excluded here: when
  // metadata is sparse, the scraper falls back to "best guess by title text",
  // and two unrelated recordings that happen to share a title (e.g. a
  // Cantonese and a Mandarin version of the same song) could both land on
  // that same guess — that must not be read as confirmation they're the same
  // recording.
  const mbMap = new Map();
  for (const f of metaFiles) {
    if (!f.mb_recording_id || f.scrape_match_basis !== 'exact') continue;
  ...
  for (const group of mbMap.values()) {
    if (group.length < 2) continue;
    // F9: pairwise (not just group[0] vs rest) + the same plausibility gate
    // as comparePair — sharing an mb_recording_id is strong evidence, but if
    // the scrape match itself was wrong (see selectMatch in scraper.js),
    // trusting it blindly regardless of how different the local title/
    // artist look is exactly how unrelated songs could get merged via a
    // shared bad MusicBrainz match.
    for (let i = 0; i < group.length; i++) {
      for (let j = i + 1; j < group.length; j++) {
        const a = group[i], b = group[j];
        if (!plausiblePair(a, b)) continue;
        uf.union(a.id, b.id);
        mbConfirmedPairs.add(pairKey(a.id, b.id));
        mbConfirmed++;
      }
    }
  }
  ...
  for (const cluster of dupClusters) {
    ...
    let maxSim = 0;
    let anyMetaConfirmed = false, anyMbConfirmed = false, anyCpConfirmed = false;
