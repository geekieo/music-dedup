...
// comparison still runs and its result feeds the "频谱声纹一致 / 声纹相似 / 声纹不同"
// label, but that label is informational only, never a gate that excludes a group.
//
// THRESHOLD: The user-configurable "频谱声纹相似度阈值" (settings.threshold, default 90%)
// controls the minimum fingerprintSimilarity() score for Phases 3 & 4 to union a pair,
// and determines whether the group gets tagged "声纹相似" vs "声纹不同". It does NOT
// affect Chromaprint — Chromaprint uses its own hardcoded CP_THRESH (see Phase 5).
...
export async function runMatcher(db, opts = {}) {
  ...
  if (metaConfirmed) onProgress({ phase: 'matching', pct: 94, message: `阶段2b完成：属性确认（标题+艺术家+时长）匹配 ${metaConfirmed} 对` });
  ...
  // ── Phase 5: Chromaprint-based local match ────────────────────────────
  ...
  onProgress({ phase: 'matching', pct: 98,
    message: cpFiles.length >= 2
      ? `阶段5完成：Chromaprint 本地匹配，${cpFiles.length} 个文件参与，${cpGroups} 组`
      : `阶段5跳过：无 Chromaprint 声纹（未安装/未配置 fpcalc）` });
...
export async function runBasicMatcher(db, opts = {}) {
  ...
  onProgress({
    phase: 'matching', pct: 80,
    message: `阶段2b完成：属性确认（标题+艺术家+时长）匹配 ${metaConfirmed} 对`,
  });
