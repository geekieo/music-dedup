...
import { detectGroupType, detectGroupTags, MATCHING_METHOD_KEYS } from './rules.js';
import { getAllFilesForMatching, clearGroups, insertGroup, insertGroupTrack, setGroupSmartKeep, runTx } from './db.js';
...
// excludeTags: optional Set of method-tag strings — groups that have ANY of
// these tags are skipped (used by force re-run to clear a lane's old groups
// without touching the DB until persist time).
function loadExistingGroups(db, uf, fileById, excludeTags = null) {
  const rows = db.all('SELECT gt.group_id, gt.file_id, g.group_tags FROM group_tracks gt JOIN dup_groups g ON g.id=gt.group_id ORDER BY gt.group_id');
  if (!rows.length) return 0;
  const groupMap = new Map();
  for (const row of rows) {
    if (!groupMap.has(row.group_id)) {
      const tags = (row.group_tags || '').split(',').filter(Boolean).filter(t => MATCHING_METHOD_KEYS.has(t));
      groupMap.set(row.group_id, { fileIds: [], tags: new Set(tags) });
    }
    groupMap.get(row.group_id).fileIds.push(row.file_id);
  }
  let count = 0;
  for (const [, { fileIds, tags }] of groupMap) {
    if (excludeTags && tags.size > 0 && [...tags].some(t => excludeTags.has(t))) continue;
...
function persistClusters(db, uf, fileById, opts) {
  ...
  // clearLaneTags strips old tags from the lane being force-re-run, so stale
  // tags don't leak into the new union.
  ...
  for (const g of oldGroups) {
    ...
    if (trackIds.length >= 2) {
      let methodTags = (g.group_tags || '').split(',').filter(Boolean).filter(t => MATCHING_METHOD_KEYS.has(t));
      if (clearLaneTags) methodTags = methodTags.filter(t => !clearLaneTags.has(t));
      if (methodTags.length) prevMethodTags.set(trackIds.join(','), new Set(methodTags));
    }
  }

  const dupClusters = uf.clusters();
  onProgress({ pct: 95, message: `发现 ${dupClusters.length} 个重复组，写入数据库...` });
  ...
  for (const cluster of dupClusters) {
    ...
    runTx(db, () => {
      // Simple quality-based pick for display (smart_keep_file_id). The
      // real retention cascade is computed on-the-fly in /api/duplicates/:id
      // using current settings — scan time is too early to lock it in.
      const sorted = [...clusterFiles].sort((a, b) => (b.bitrate || 0) - (a.bitrate || 0));
      const smartFileId = sorted[0]?.id || null;
      ...
      setGroupSmartKeep(db, groupId, smartFileId);
      // Compute savings: sum of non-kept tracks
      const keepIds = new Set(smartFileId ? [smartFileId] : []);
...
export async function runScrapeMatcher(db, opts = {}) {
  ...
  const preservedCount = loadExistingGroups(db, uf, fileById, clearLaneTags);
...
export async function runBasicMatcher(db, opts = {}) {
  ...
  for (const group of titleGroupList) {
    ...
    titleGroupDone++;
    if (titleGroupDone % 200 === 0) {
      prog.emit({ phase:'basicMatch', pct:40 + Math.round(titleGroupDone/Math.max(1,titleGroupTotal)*40), subPct:Math.round(titleGroupDone/Math.max(1,titleGroupTotal)*100), level:'ok',
        message:`属性确认: ${titleGroupDone} / ${titleGroupTotal} 组（已确认 ${metaConfirmed} 对）` });
    }
  }

  prog.emit({
    phase: 'basicMatch', pct: 80, subPct: 100,
...
export async function runFpMatcher(db, opts = {}) {
  ...
  let prefixGroups = 0;
  const prefixKeys = [...prefixMap.keys()];
  const prefixTotal = prefixKeys.length;
  for (const key of prefixKeys) {
    const group = prefixMap.get(key);
    if (group.length < 2) continue;
    compareGroup(group);
    prefixGroups++;
    if (prefixGroups % 50 === 0) {
      prog.emit({ phase:'fpMatch', pct:30 + Math.round(prefixGroups/Math.max(1,prefixTotal)*20), subPct:Math.round(prefixGroups/Math.max(1,prefixTotal)*100), level:'ok',
        message:`频谱多段LSH分组: ${prefixGroups} / ${prefixTotal} 组` });
      await prog.flush();
    }
  }
  prog.emit({ phase:'fpMatch', pct: 50, subPct: 100, message: `频谱多段LSH分组完成：${prefixGroups} 组` });
  ...
  const durEntries = [...durMap.values()];
  const durTotal = durEntries.length;
  for (const group of durEntries) {
    if (group.length < 2) continue;
    ...
    if (++durLoop % 50 === 0) {
      prog.emit({ phase:'fpMatch', pct:50 + Math.round(durLoop/Math.max(1,durTotal)*25), subPct:Math.round(durLoop/Math.max(1,durTotal)*100), level:'ok',
        message:`频谱时长桶匹配: ${durLoop} / ${durTotal} 组（${durGroups} 有效${durSkipped?`，${durSkipped} 超限跳过`:''}）` });
      await prog.flush();
    }
  }
  prog.emit({ phase:'fpMatch', pct: 75, subPct: 100, message: `频谱时长桶匹配完成：${durGroups} 组${durSkipped?`（另有 ${durSkipped} 个超大分组递归细分后仍超限，已跳过）`:''}` });
  ...
  if (cpFiles.length >= 2) {
    ...
    const cpPrefixEntries = [...cpPrefixMap.entries()];
    const cpPrefixTotal = cpPrefixEntries.length;
    for (const [key, group] of cpPrefixEntries) {
      if (group.length < 2) continue;
      compareCpGroup(group);
      cpLshGroups++;
      if (cpLshGroups % 50 === 0) {
        prog.emit({ phase:'fpMatch', pct:78 + Math.round(cpLshGroups/Math.max(1,cpPrefixTotal)*4), subPct:Math.round(cpLshGroups/Math.max(1,cpPrefixTotal)*100), level:'ok',
          message:`CP多段LSH分组: ${cpLshGroups} / ${cpPrefixTotal} 组` });
        await prog.flush();
      }
    }
    prog.emit({ phase:'fpMatch', pct: 82, subPct: 100, message: `CP多段LSH分组完成：${cpLshGroups} 组` });
    ...
    const cpDurEntries = [...cpDurMap.values()];
    const cpDurTotal = cpDurEntries.length;
    for (const group of cpDurEntries) {
      if (group.length < 2) continue;
      if (group.length > DUR_BUCKET_LIMIT) {
      ...
      if (++cpDurLoop % 20 === 0) {
        prog.emit({ phase:'fpMatch', pct:82 + Math.round(cpDurLoop/Math.max(1,cpDurTotal)*5), subPct:Math.round(cpDurLoop/Math.max(1,cpDurTotal)*100), level:'ok',
          message:`CP时长桶匹配: ${cpDurLoop} / ${cpDurTotal} 组（${cpDurGroups} 有效${cpDurSkipped?`，${cpDurSkipped} 超限`:''}）` });
        await prog.flush();
      }
    }
    prog.emit({ phase:'fpMatch', pct: 87, subPct: 100, message: `CP时长桶匹配完成：${cpDurGroups} 组${cpDurSkipped?`（${cpDurSkipped} 个超限）`:''}` });
  }
