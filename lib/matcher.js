...
//            5b  LSH分组                     声纹前缀哈希分组 → 候选对
//            5c  时长桶+相似度               时长±4s分桶 + fingerprintSimilarity()
//   步骤 6  CP声纹匹配        matcher.js    runFpMatcher Chromaprint 部分
//                                           chromaprintSimilarity() ≥ CP_THRESH
...
function persistClusters(db, uf, fileById, opts) {
  ...
  for (const cluster of dupClusters) {
    ...
    runTx(db, () => {
      ...
      const final_tags = hasTie ? match_tags + ',retention_tie' : match_tags;
...
export async function runMatcher(db, opts = {}) {
  ...
  onProgress({ phase: 'matching', pct: 92, message: `步骤5a完成：精确声纹匹配 ${exactCount} 对` });
  ...
  onProgress({ phase: 'matching', pct: 93, message: `步骤3a完成：标题分组 ${titleGroups} 组` });
  ...
  if (metaConfirmed) onProgress({ phase: 'matching', pct: 94, message: `步骤3b完成：属性确认（标题+艺术家+时长）匹配 ${metaConfirmed} 对` });
  ...
  if (mbConfirmed||acoustidConfirmed) onProgress({ phase: 'matching', pct: 94, message: `步骤8完成：刮削匹配 ${mbConfirmed+acoustidConfirmed} 对（MusicBrainz ${mbConfirmed}，AcoustID ${acoustidConfirmed}）` });
  ...
  onProgress({ phase: 'matching', pct: 96, message: `步骤5b完成：LSH分组 ${prefixGroups} 组` });
  ...
  for (const group of durMap.values()) {
    if (group.length < 2) continue;
    // Skip only extremely large buckets (>600) — smaller caps silently dropped
    // genuine duplicates in libraries with many short tracks (e.g. anime OSTs).
    if (group.length > 600) { durSkipped++; continue; }
    compareGroup(group);
    durGroups++;
  }
  onProgress({ phase: 'matching', pct: 97, message: `步骤5c完成：时长桶匹配 ${durGroups} 组${durSkipped?`（另有 ${durSkipped} 个超大分组已跳过，未比较）`:''}` });
  ...
  // spectral phases above: uses its own hardcoded CP_THRESH (0.90), NOT the
  // user-configurable spectral threshold. Unions its own pairs and tags them
  // separately (cp_confirmed) so the resulting duplicate groups make it
  // possible to compare the two fingerprinting methods' results (see which
  // groups only one of them found) rather than silently blending them together.
  ...
  onProgress({ phase: 'matching', pct: 98,
    message: cpFiles.length >= 2
      ? `步骤6完成：CP声纹匹配，${cpFiles.length} 个文件参与，${cpGroups} 组`
      : `步骤6跳过：无 CP 声纹（未安装/未配置 fpcalc）` });
...
export async function runScrapeMatcher(db, opts = {}) {
  ...
  onProgress({
    ...
    message: `步骤8完成：刮削匹配 ${mbConfirmed + acoustidConfirmed} 对（MusicBrainz ${mbConfirmed}，AcoustID ${acoustidConfirmed}）`,
  });
...
export async function runBasicMatcher(db, opts = {}) {
  ...
  onProgress({ phase: 'matching', pct: 40, message: `步骤3a完成：标题分组 ${titleGroups} 组` });
  ...
  onProgress({
    phase: 'matching', pct: 80,
    message: `步骤3b完成：属性确认（标题+艺术家+时长）匹配 ${metaConfirmed} 对`,
  });
...
export async function runFpMatcher(db, opts = {}) {
  ...
  onProgress({ phase: 'matching', pct: 30, message: `步骤5a完成：精确声纹匹配 ${exactCount} 对` });
  ...
  onProgress({ phase: 'matching', pct: 50, message: `步骤5b完成：LSH分组 ${prefixGroups} 组` });
  ...
  onProgress({ phase: 'matching', pct: 75, message: `步骤5c完成：时长桶匹配 ${durGroups} 组${durSkipped?`（另有 ${durSkipped} 个超大分组已跳过，未比较）`:''}` });
