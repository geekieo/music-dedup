...
export async function runMatcher(db, opts = {}) {
  ...
  onProgress({ phase: 'matching', pct: 96, message: `步骤5b完成：多段LSH分组 ${prefixGroups} 组` });
  ...
  onProgress({ phase: 'matching', pct: 97, message: `步骤5c完成：时长桶匹配 ${durGroups} 组${durSkipped?`（另有 ${durSkipped} 个超大分组递归细分后仍超限，已跳过）`:''}` });
  ...
  onProgress({ phase: 'matching', pct: 98,
    message: cpFiles.length >= 2
      ? `步骤6完成：CP声纹匹配，${cpFiles.length} 个文件参与，LSH ${cpLshGroups} + 时长 ${cpDurGroups} 组${cpDurSkipped?`（${cpDurSkipped} 个超限）`:''}`
...
export async function runFpMatcher(db, opts = {}) {
  ...
  onProgress({ phase: 'matching', pct: 50, message: `步骤5b完成：多段LSH分组 ${prefixGroups} 组` });
  ...
  onProgress({ phase: 'matching', pct: 75, message: `步骤5c完成：时长桶匹配 ${durGroups} 组${durSkipped?`（另有 ${durSkipped} 个超大分组递归细分后仍超限，已跳过）`:''}` });
