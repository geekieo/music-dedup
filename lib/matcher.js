...
export async function runScrapeMatcher(db, opts = {}) {
  ...
  const prog = new Progress(onProgress, 'scrapeMatch');

  prog.emit({ phase: 'scrapeMatch', pct: 0, message: '开始刮削匹配分析...' });
  ...
  if (metaFiles.length === 0) {
    prog.emit({ phase: 'done', pct: 100, message: '没有可匹配的文件' });
    return;
  }
  ...
  if (preservedCount) prog.emit({ phase: 'scrapeMatch', pct: 20, message: `已保留 ${preservedCount} 个现有重复组，合并刮削数据...` });
  ...
  const { inserted, dupBytes } = persistClusters(db, uf, fileById, {
    ...
    qualityTiers, onProgress: (evt) => prog.emit(evt), onAbort,
  });
...
export async function runBasicMatcher(db, opts = {}) {
  ...
  const prog = new Progress(onProgress, 'basicMatch');

  prog.emit({ phase: 'basicMatch', pct: 0, message: '开始基础匹配分析...' });
  ...
  // ── Preserve existing groups ──────────────────────────────────────────
  const preservedCount = loadExistingGroups(db, uf, fileById);
  if (preservedCount) prog.emit({ phase: 'basicMatch', pct: 20, message: `已保留 ${preservedCount} 个现有重复组，合并基础匹配...` });
...
export async function runFpMatcher(db, opts = {}) {
  ...
  const prog = new Progress(onProgress, 'fpMatch');
  prog.emit({ phase: 'fpMatch', pct: 0, message: '开始声纹匹配分析...' });
  ...
  // ── Preserve existing groups ──────────────────────────────────────────
  const preservedCount = loadExistingGroups(db, uf, fileById);
  if (preservedCount) prog.emit({ phase: 'fpMatch', pct: 15, message: `已保留 ${preservedCount} 个现有重复组，合并声纹匹配...` });
  ...
  prog.emit({ phase: 'fpMatch', pct: 30, message: `精确声纹匹配完成：${exactCount} 对` });
  ...
  prog.emit({ phase: 'fpMatch', pct: 50, message: `多段LSH分组完成：${prefixGroups} 组` });
  ...
  prog.emit({ phase: 'fpMatch', pct: 75, message: `时长桶匹配完成：${durGroups} 组${durSkipped?`（另有 ${durSkipped} 个超大分组递归细分后仍超限，已跳过）`:''}` });
