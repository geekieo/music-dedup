...
import { clearGroups, insertGroup, insertGroupTrack, setGroupSmartKeep } from './db/groups.js';
...
// P5 联调：改为 async —— compareFn（compareGroup/compareCpGroup）现在协作式让出事件循环。
async function subdivideDurBucket(group, getSegmentKeys, compareFn, depth = 0) {
  if (group.length <= DUR_BUCKET_LIMIT || depth >= MAX_SUBDIVIDE_DEPTH) {
    if (group.length <= DUR_BUCKET_LIMIT) await compareFn(group);
  ...
  for (const sub of subMap.values()) {
    ...
    if (!await subdivideDurBucket(sub, getSegmentKeys, compareFn, depth + 1)) allOk = false;
  }
  return allOk;
}

// ── P5 联调：匹配阶段协作式让出事件循环 ──────────────────────────────────────
// 匹配阶段是纯 CPU 同步（相似度对比 × 大量文件对，实测 fpMatch 最大单块阻塞 27s），
// 在主进程线程跑会让窗口"未响应"。maybeYield 每 ~YIELD_INTERVAL_MS 让出一次
// （setImmediate → macrotask → 事件循环泵出窗口消息/拖动）。算法与结果不变。
...
// 阈值收敛到 lib/constants.js 单一出处；局部别名保留 matcher 上下文语义。
...
// P5 联调：async + 时间切片让出（大组 maxSim 对 + 每簇 detectGroupType/tags 都是同步 CPU）
...
export async function runScrapeMatcher(db, opts = {}) {
  ...
  const y = { t: Date.now() }; // P5 联调：时间切片让出
...
export async function runBasicMatcher(db, opts = {}) {
  ...
  const y = { t: Date.now() }; // P5 联调：时间切片让出，超大同名组不阻塞窗口
...
export async function runFpMatcher(db, opts = {}) {
  ...
  // P5 联调：异步化 + 时间切片让出（热桶/热时长的成对对比是同步 CPU 大户）
  ...
  for (const group of durEntries) {
    ...
    if (group.length > DUR_BUCKET_LIMIT) {
      if (!await subdivideDurBucket(group, durKeyFn, compareGroup)) durSkipped++;
      else durGroups++;
    } else {
      await compareGroup(group);
      durGroups++;
  ...
  // P5 联调：异步化 + 时间切片让出（chromaprint 原始数组相似度最重，实测是冻结主凶）
  async function compareCpGroup(group) {
    const y = { t: Date.now() };
    for (let i = 0; i < group.length && !onAbort(); i++) {
      for (let j = i + 1; j < group.length; j++) {
        const a = group[i], b = group[j];
  ...
  if (cpFiles.length >= 2) {
    ...
    for (const group of cpDurEntries) {
      if (group.length < 2) continue;
      if (group.length > DUR_BUCKET_LIMIT) {
        if (!await subdivideDurBucket(group, cpKeyFn, compareCpGroup)) cpDurSkipped++;
        else cpDurGroups++;
      } else {
        await compareCpGroup(group);
        cpDurGroups++;
  ...
  prog.emit({
    phase: 'done', pct: 100, level: 'done',
    message: `声纹匹配完成！发现 ${inserted} 个重复组（频谱精确 ${exactCount} 对 + CP 确认 ${cpConfirmedPairs.size} 对），可释放 ${fmtBytes(dupBytes)}`,
    groups: inserted, savings: dupBytes,
  });
}
