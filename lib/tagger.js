...
export async function writeTagsWithSnapshot(db, fileId, fieldsToWrite) {
  ...
  try {
    ...
    if (existing) {
      db.run(`UPDATE write_history SET current_tags=?, modified_fields=JSON(
          '[' || (CASE WHEN modified_fields='[]' THEN '' ELSE SUBSTR(modified_fields,2,LENGTH(modified_fields)-2)||',' END) ||
          SUBSTR(?,2,LENGTH(?)-2) || ']'),
        write_count=write_count+1, last_written_at=? WHERE file_id=?`,
        [JSON.stringify(afterRaw),
         JSON.stringify(modifiedFields), JSON.stringify(modifiedFields),
         now, fileId]);
    } else {
      db.run(`INSERT INTO write_history
        (file_id,file_path,file_title,file_artist,original_tags,current_tags,modified_fields,write_count,first_written_at,last_written_at)
        VALUES(?,?,?,?,?,?,?,1,?,?)`,
        [fileId, file.path, file.title||null, file.artist||null,
         JSON.stringify(originalRaw), JSON.stringify(afterRaw),
         JSON.stringify(modifiedFields), now, now]);
    }
