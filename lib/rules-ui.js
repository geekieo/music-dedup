...
export const GROUP_TAG_LABELS = {
  ...
  metadata_same: '标签一致', metadata_diff: '标签不同', duration_same: '时长一致', duration_diff: '时长不同',
  release_year_diff: '年份不同', release_type_diff: '发行类型不同', meta_score_diff: '标签完整度不同', retention_tie: '保留平局',
};
...
export const GROUP_TAG_DESCRIPTIONS = {
  ...
  meta_score_diff: '组内标签完整度不同，优先保留字段更全的文件。',
...
export const DIMENSION_DEFS = [
  { key: 'duration_accurate', label: '时长', icon: 'clock',
    cell: (t, all) => {
      ...
      return { text: t._durationAccurate ? `${mmss(t.duration)} ✓` : `${mmss(t.duration)} (偏差${dev}s)`, ok };
    }},
  ...
  { key: 'meta_best', label: '标签完整度', icon: 'list-check',
    cell: (t, all) => {
...
// 刮削档位展示（TIER_COLOR/LABEL/DESC）在 lib/tier.js（单一出处）。
