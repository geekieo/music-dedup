...
export function statsQuery(db) {
  const total      = (db.get('SELECT COUNT(*) n FROM files WHERE deleted=0')||{n:0}).n;
  const albums     = (db.get("SELECT COUNT(DISTINCT album) n FROM files WHERE deleted=0 AND album IS NOT NULL AND album!=''")||{n:0}).n;
  const artists    = (db.get("SELECT COUNT(DISTINCT artist) n FROM files WHERE deleted=0 AND artist IS NOT NULL AND artist!=''")||{n:0}).n;
  const totalBytes = (db.get('SELECT COALESCE(SUM(size),0) s FROM files WHERE deleted=0')||{s:0}).s;
  const formats    = db.all('SELECT format, COUNT(*) n FROM files WHERE deleted=0 AND format IS NOT NULL GROUP BY format ORDER BY n DESC');
  const withMeta   = (db.get('SELECT COUNT(*) n FROM files WHERE deleted=0 AND meta_extracted_at IS NOT NULL')||{n:0}).n;
  const withFP     = (db.get('SELECT COUNT(*) n FROM files WHERE deleted=0 AND fingerprint IS NOT NULL')||{n:0}).n;
...
function queryLibraryAllForTier(db, { search='', format='', libFilter='all' }={}) {
  const whereParts = [];
  const params = [];
  whereParts.push('f.deleted=0'); // 回收站文件不进音乐库
...
export function locateFileInLibrary(db, fileId, { sort = 'title', order = 'asc' } = {}) {
  ...
  const rows = db.all(`SELECT id FROM files WHERE deleted=0 ORDER BY ${safeSort} ${safeOrder} NULLS LAST`);
