// lib/db/index.js — SQLite (WASM) 连接初始化、schema 与迁移（db.js 按域拆分后的入口）
...
export function openDB({ skipInit = false, path: dbPath = null, skipPragma = false } = {}) {
  // path 显式传入时用指定文件（scan worker 用主库的 VACUUM INTO 快照副本，P5.1 锁冲突修复）；
  // 缺省仍为 DB_PATH（主进程单连接）。skipPragma：worker 的临时库单连接、不需要并发 pragma，
  // 且 worker 线程对 VACUUM 产物执行 journal_mode=WAL 会异常报 database is locked（实测）。
  const db = new Database(dbPath || DB_PATH);
  if (!skipPragma) {
    // busy_timeout：扫描 worker 化后主进程与 worker 双连接并发写（WAL 单写者），
    // 扫描长事务期间主进程写标签等短写等待重试而非立即 SQLITE_BUSY。
    // node-sqlite3-wasm 的 Database 构造器无 timeout 选项，只能靠 pragma。
    // 注：本 WASM 编译版不支持 WAL（PRAGMA journal_mode=WAL 静默回退 delete），
    // 双连接在 DELETE 模式下仍会互锁——故扫描 worker 改走独立临时库副本（见 electron/ipc/scan.js）。
