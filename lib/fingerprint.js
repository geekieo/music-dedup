...
export function fingerprintSimilarity(fp1, fp2) {
  ...
  return spectralSim(fp1, fp2);
}

function spectralSim(fp1, fp2) {
  const a = fp1.trim().split(/\s+/).map(Number);
  const b = fp2.trim().split(/\s+/).map(Number);
  const len = Math.min(a.length, b.length);
  if (len === 0) return 0;
  let bits = 0;
  for (let i = 0; i < len; i++) bits += 32 - hammingWeight((a[i] ^ b[i]) >>> 0);
  return bits / (len * 32);
}
...
function metaSim(fp1, fp2) {
  ...
  return ts >= 0.75 ? Math.min(ts, 0.96) : 0;
}

// ── Chromaprint similarity (experimental local matching) ─────────────────
// Compares the RAW (uncompressed) integer arrays from fpcalc -raw — see
// lib/chromaprint-bridge.js. Same bit-difference approach as spectralSim
// above: this is a simple whole-fingerprint alignment (assumes both audios
// start at roughly the same point), not the windowed/offset-searching
// alignment a full Chromaprint/AcoustID match implementation would do — a
// reasonable first-pass approximation for comparing against the built-in
// spectral fingerprint's results, not a production-grade audio matcher.
export function chromaprintSimilarity(raw1, raw2) {
...
export function normalizeStr(s) {
  return (s || '')
    ...
    // "Micmacs A La Gare" normalize to DIFFERENT strings ("la gare" vs "a la
    // gare" — the accented "à" used to be deleted as punctuation instead of
    // folding to "a"), which silently breaks title-based duplicate matching
    // for any file with accented characters in its tags or filename.
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    // Keep any Unicode letter/number (covers CJK, Cyrillic, Hangul, etc. —
    // not just ASCII \w + the old hardcoded CJK range) plus underscore.
