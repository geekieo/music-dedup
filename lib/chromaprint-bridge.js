...
//   2. Electron resources dir (process.resourcesPath — packaged v2 app).
...
function candidates(customPath) {
  ...
  // v2 打包后 fpcalc.exe 经 electron-builder extraResources 落地 resources/；
  // 项目根候选在 asar 内（execFile 无法从 asar 启动进程），故补充 resourcesPath。
  // 纯 Node（v1）下 process.resourcesPath 为 undefined，带保护不影响。
  return [
...
export async function computeChromaprint(filePath, customPath = '') {
  ...
  try { return await run(filePath); }
    ...
    // 失败不静默（P5.1/C1）：返回 {error} 标记，调用方（runFingerprint）计数 + 记日志，
