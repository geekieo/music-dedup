...
export async function runEnumerate(db, { dirs=[], exclude=[], onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  prog.emit({ phase:'enum', pct:2, level:'info', message:'开始文件枚举...' });
  await onPause();
...
export async function runMetadata(db, { threads=8, smartScan=true, onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  const prog = new Progress(onProgress);
  const log = createPhaseLog(prog, { phase: 'meta', label: '读取标签' });

  if (total === 0) {
    log.start(0, '无需更新：所有文件均未修改');
    log.done('读取标签完成');
    return 0;
  }

  log.start(total, `${total.toLocaleString()} 个文件`);
  ...
  log.done(errs ? `读取标签完成（${errs} 个错误）` : '读取标签完成');
  return done;
}
...
export async function runFingerprint(db, { threads=8, smartScan=true, fpcalcPath='', fpCompute, fpComputeParallel=false, onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  const prog = new Progress(onProgress);
  const log = createPhaseLog(prog, { phase: 'fp', label: '声纹提取' });

  if (total === 0) {
    log.start(0, fpcalc
      ? '无需更新：频谱声纹 + Chromaprint 声纹均已完整'
      : '无需更新：所有文件均未修改');
    log.done('声纹提取完成');
    return 0;
  }
  ...
  log.start(total, `${total.toLocaleString()} 个文件${hint}`);
        ...
        cpTasks.push(cpSlot().then(async (acquired) => {
          if (acquired && !onAbort()) {
            try {
              ...
              else if (cp?.error) { errs++; if (errs <= 5) console.log(`[scanner] Chromaprint 失败 (${f.path}): ${cp.error}`); }
            } catch (e) { errs++; if (errs <= 5) console.log(`[scanner] Chromaprint 失败 (${f.path}): ${e.message}`); }
          }
          if (acquired) cpRelease();
        }));
      }
    } catch (e) { errs++; if (errs <= 5) console.log(`[scanner] 声纹提取失败 (${f.path}): ${e.message}`); }
    done++;
    log.tick(done);
  };
  ...
  log.done(errs ? `声纹提取完成（${errs} 个错误）` : '声纹提取完成');
  return done;
}
