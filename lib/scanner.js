...
import {
  upsertFileBasic, updateFileMeta, updateFileFingerprint,
...
// ── Phase 1: Enumerate ────────────────────────────────────────────────────
export async function runEnumerate(db, { dirs=[], exclude=[], onProgress=()=>{}, onAbort=()=>false }={}) {
...
export async function runMetadata(db, { threads=8, smartScan=true, onProgress=()=>{}, onAbort=()=>false }={}) {
  ...
  for (let i = 0; i < files.length; i += batch) {
    ...
    const results = await Promise.all(slice.map(f => _extractMeta(f.path).catch(()=>null)));
...
export async function runFingerprint(db, { threads=8, smartScan=true, onProgress=()=>{}, onAbort=()=>false }={}) {
  ...
  onProgress({ phase:'fp', pct:0, message:`开始提取声纹（${total.toLocaleString()} 个文件）...` });
