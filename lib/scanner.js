...
export async function runMetadata(db, { threads=8, smartScan=true, onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  if (total === 0) {
    prog.emit({ phase:'meta', pct:100, level:'ok', message:'文件属性提取: 无需更新（智能扫描：所有文件均未修改）' });
    prog.done('文件属性提取完成');
    return 0;
  }
...
// ── 步骤4: 提取声纹 ──────────────────────────────────────────────────────
export async function runFingerprint(db, { threads=8, smartScan=true, fpcalcPath='', fpCompute, onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  // P5：fpCompute 可注入自定义 compute（原 electron/fp-worker.js 的 audio-decode offload
  // 已随扫描整体移入 electron/scan-worker.js 线程，缺省即本地 computeFingerprint——worker
  // 线程本身就是隔离）。无注入时保持 lib 自包含可测。
  ...
  if (total === 0) {
    prog.emit({ phase:'fp', pct:100, level:'ok', message: fpcalc
  ...
  for (let i = 0; i < allFiles.length; i += batch) {
    ...
    // 解码串行：音频解码是单线程 WASM，Promise.all 并发不会加速、反因内存压力更慢
    //（实测 8 文件并发 13.6s > 串行 7.5s），且同步解码阻塞本线程会挡住中止消息。
    // 逐文件 abort/pause 检查：STOP 至多等一个文件解码完（~2s），不再等整批。
    // fpcalc 是子进程，逐个解码完即触发、与后续解码自然错峰，批末统一等齐。
    const cpTasks = [];
    for (const f of slice) {
      if (onAbort()) { await Promise.all(cpTasks); return done; }
      await onPause();
      try {
        const needsGoertzel = !chromaOnlySet.has(f.id);
        if (needsGoertzel) {
          const { fingerprint, duration, method } = await compute(f.path);
          updateFileFingerprint(db, f.id, fingerprint, method, duration);
        }
        if (fpcalc && (!f.chromaprint || !f.chromaprint_raw)) {
          cpTasks.push(computeChromaprint(f.path, fpcalcPath)
            .then(cp => { if (cp?.fingerprint) updateFileChromaprint(db, f.id, cp.fingerprint, cp.fingerprintRaw); })
            .catch(() => {}));
        }
      } catch { errs++; }
      done++;
