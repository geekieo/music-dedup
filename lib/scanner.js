...
export async function runFingerprint(db, { threads=8, smartScan=true, onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  const files = getFilesNeedingFP(db, smartScan);
  const total = files.length;

  if (total === 0) {
    onProgress({ phase:'fp', pct:100, message:'声纹无需更新（智能扫描：所有文件均未修改）' });
    return 0;
  }

  // Check for fpcalc once so we can report it upfront
  const fpcalc = await detectFpcalc();
  const hintStr = fpcalc ? ' + Chromaprint(AcoustID)' : ' (未检测到 fpcalc，跳过 Chromaprint)';
  onProgress({ phase:'fp', pct:0, message:`开始提取声纹（${total.toLocaleString()} 个文件）${hintStr}...` });
  ...
  for (let i = 0; i < files.length; i += batch) {
    ...
    await Promise.all(slice.map(async f => {
      try {
        // Internal Goertzel fingerprint (for duplicate detection similarity)
        ...
        // Chromaprint fingerprint (for AcoustID lookup) — only when fpcalc is available
        if (fpcalc) {
          try {
            const cp = await computeChromaprint(f.path);
            if (cp?.fingerprint) updateFileChromaprint(db, f.id, cp.fingerprint);
          } catch { /* fpcalc failure is non-fatal */ }
        }
      } catch { errs++; }
    }));
