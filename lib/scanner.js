...
export async function runEnumerate(db, { dirs=[], exclude=[], onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  onProgress({ phase:'done', pct:100, message:'文件枚举完成' });
...
export async function runMetadata(db, { threads=8, smartScan=true, onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  if (total === 0) {
    onProgress({ phase:'meta', pct:100, message:'文件属性无需更新（智能扫描：所有文件均未修改）' });
    onProgress({ phase:'done', pct:100, message:'文件属性提取完成' });
    return 0;
  }

  onProgress({ phase:'meta', pct:0, message:`开始提取文件属性（${total.toLocaleString()} 个文件）...` });
  ...
  for (let i = 0; i < files.length; i += batch) {
    ...
    const results = await Promise.all(slice.map(async f => {
      ...
      onProgress({ phase:'meta', pct: Math.round(done/total*100),
        message:`文件属性: ${done.toLocaleString()} / ${total.toLocaleString()}${errs?' ('+errs+' 错误)':''}`,
        filesProcessed: done });
      return r;
    }));
...
export async function runFingerprint(db, { threads=8, smartScan=true, fpcalcPath='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  // — OR have the OLD compressed fingerprint but are still missing the RAW
  // one (chromaprint_raw was added after chromaprint; only checking
  // `chromaprint IS NULL` meant a file that already got its compressed
  // fingerprint in an earlier scan could never be picked up again to
  // backfill chromaprint_raw, which is exactly why local Chromaprint
  // matching kept reporting "无 Chromaprint 声纹" even after fpcalc was
  // clearly working (AcoustID hits proved the compressed fingerprint existed).
  ...
  if (total === 0) {
    onProgress({ phase:'fp', pct:100, message: fpcalc
      ? '声纹无需更新（频谱声纹 + Chromaprint 声纹均已完整）'
      : '声纹无需更新（智能扫描：所有文件均未修改）' });
    onProgress({ phase:'done', pct:100, message:'声纹提取完成' });
    return 0;
  }
