...
export function detectMatchTags(tracks, similarity, extra = {}) {
  const tags = new Set();
  const { spectralConfirmed = false, mbConfirmed = false, cpConfirmed = false } = extra;
  ...
  // F9 (experimental): confirmed via Chromaprint (fpcalc) fingerprint
  // comparison, run alongside — not instead of — the built-in spectral
  // fingerprint above, so groups can be compared to see which method(s)
  // actually caught them. Deliberately its own tag rather than folded into
  // exact_copy/same_recording/fp_diff, which describe the spectral signal.
  if (cpConfirmed) tags.add('cp_confirmed');
