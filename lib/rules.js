 ...
 */
export function detectMatchTags(tracks, similarity) {
  const tags = new Set();

  // Exact fingerprint copy
  ...
  // Same metadata (title + artist)
  const metaKeys = tracks.map(t =>
    `${(t.title||'').toLowerCase().trim()}__${(t.artist||'').toLowerCase().trim()}`);
  ...
  // Duration near (≤5s difference)
  const durs = tracks.map(t => Math.round(t.duration || 0)).filter(d => d > 0);
  if (durs.length >= 2 && Math.max(...durs) - Math.min(...durs) <= 5)
    tags.add('duration_near');

  // High-similarity but not exact (same recording, different encode)
