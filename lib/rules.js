...
export function detectMatchTags(tracks, similarity, extra = {}) {
  const tags = new Set();
  const {
    spectralConfirmed = false, mbConfirmed = false, acoustidConfirmed = false,
    cpConfirmed = false, maxCpSim = 0, hasCpData = false,
  } = extra;
  ...
  // F9: Chromaprint's "exact" isn't a byte-string equality check like the
  // built-in spectral fingerprint (fpcalc's raw output can vary by a few
  // bits between runs even for identical audio) — near-certain similarity
  // (≥98%) is the practical equivalent.
  const CP_EXACT = 0.98;
  const cpExact = hasCpData && maxCpSim >= CP_EXACT;

  // F9: split what used to be one "声纹一致"/"声纹相似"/"声纹不同" triple
  // (which only ever described the built-in spectral fingerprint) into
  // separate spectral/Chromaprint tags, since a file can now have either,
  // both, or neither. "声纹不同" is deliberately the ONLY tag that requires
  // checking both: it should mean "neither available fingerprinting method
  // found any similarity", not just "the spectral one didn't" — if
  // Chromaprint isn't available at all for this cluster, it's judged on the
  // spectral signal alone, same as before.
  if (spectralExact) tags.add('spectral_exact');
  else if (spectralConfirmed) tags.add('same_recording');
  if (cpExact) tags.add('cp_exact');
  const spectralFoundNothing = !spectralConfirmed; // covers exact too (exact implies confirmed)
  ...
  // Two files independently scraped to the SAME external recording id.
  // AcoustID's is audio-fingerprint-verified (higher confidence); a plain
  // MusicBrainz text search landing on the same id is weaker corroboration
  // — kept as two separate tags rather than one so the group's confidence
