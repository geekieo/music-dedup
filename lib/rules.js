...
// Evaluates every track in a duplicate group, assigns match tags (how was
...
export const PICK_TAG_LABEL = {
  scrape_best: '刮削更准', quality_best: '音质最优', release_best: '发行更早',
...
// ═══════════════════════════════════════════════════════════════════════════
// Group-level detection: type + match tags
// ═══════════════════════════════════════════════════════════════════════════
...
//
export function detectMatchTags(tracks, _similarity, extra = {}) {
  const tags = new Set();
  const {
...
// ═══════════════════════════════════════════════════════════════════════════
// Display data: match-tag labels, descriptions, colors (served to client)
// ═══════════════════════════════════════════════════════════════════════════
...
export const MATCH_TAG_DESCRIPTIONS = {
  ...
  fp_diff: '频谱声纹和 Chromaprint 声纹都没有比对上。',
  format_diff: '组内存在不同的文件格式（如 FLAC + MP3），保留规则根据音质优先级选择，同音质选择更大码率。',
...
// Tag pairs that never co-occur on the same group (detectMatchTags assigns
...
export const DIMENSION_DEFS = [
  { key: 'duration_accurate', label: '时长准确',
    cell: (t, all) => {
  ...
  { key: 'meta_best', label: '属性完整度',
    cell: (t, all) => {
      const max = Math.max(...all.map(x => x._meta || 0));
      return { text: `${t._meta}/100`, ok: t._meta === max };
    }},
];
