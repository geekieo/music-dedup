...
// ── Dimension tags (for UI display) ───────────────────────────────────────

function annotateStrengths(annotated) {
  if (annotated.length <= 1) return;
  ...
  const earliestYr  = Math.min(...annotated.map(t => t.album_year || 9999));
  ...
  for (const t of annotated) {
    ...
    if (t.album_year && t.album_year === earliestYr && earliestYr < 9999) tags.push('release_best');
    t._tags = tags;
  }
}
...
export function tagTracks(tracks, tierOrder) {
  ...
  const bestQuality = Math.max(...annotated.map(t => t._quality));
  const bestMeta    = Math.max(...annotated.map(t => t._meta));
  const years = annotated.map(t => t.album_year || 0).filter(y => y > 0);
  const earliestYr = years.length > 0 ? Math.min(...years) : 0;
...
export function applyRetentionRules(tracks, getLocalAlbumCount, tierOrder) {
  ...
  if (verified.length > 0) {
    ...
    if (recIds.size === 1) {
      const bestMatch = Math.max(...verified.map(t => t._scrapeMatches));
      candidates = verified.filter(t => t._scrapeMatches === bestMatch);
    }
  }

  // ── Rule 2: 音质优先 ─────────────────────────────────────────────────
  if (candidates.length > 1) {
    const maxQ = Math.max(...candidates.map(t => t._quality));
    candidates = candidates.filter(t => t._quality === maxQ);
  }
  if (candidates.length === 1) {
    return markResult(annotated, candidates[0], `优先音质 (${fmtQuality(candidates[0])})`);
  }

  // ── Rule 3: 首发优先 ─────────────────────────────────────────────────
  if (candidates.length > 1) {
    const official = candidates.filter(t => t._rtype === 'album' || t._rtype === 'soundtrack');
    const pool = official.length > 0 ? official : candidates;
    const minYear = Math.min(...pool.map(t => t.album_year || 9999));
    candidates = pool.filter(t => (t.album_year || 9999) === minYear);
  }
  if (candidates.length === 1) {
    const y = candidates[0].album_year;
    return markResult(annotated, candidates[0], `优先音质 · ${y > 0 ? `首发 ${y}` : '年份未知'}`);
  }
  ...
  if (candidates.length > 1) {
    ...
    if (tied.length === 1) {
      return markResult(annotated, tied[0], '属性最完整');
    }
    candidates = tied;
  }
