...
export const GROUP_TAG_DESCRIPTIONS = {
  ...
  duration_same: '组内文件时长一致，大概率是同一份拷贝（不同容器/编码带来的取整误差除外）。',
  duration_diff: '组内文件时长不一致。可能是不同版本，需手动核对。可能是编码误差，根据精确刮削时长择优。',
  release_year_diff: '组内文件的发行年份不同，保留规则可能优先选择首发专辑。',
  release_type_diff: '组内存在不同的发行类型（专辑/单曲/原声/合辑），保留规则优先选择专辑版。',
  meta_score_diff: '组内文件的属性完整度不同。优先保留属性字段更完整的文件。',
  retention_tie: '智能保留规则无法自动决定。',
};
...
export const DIMENSION_DEFS = [
  ...
  { key: 'quality_best', label: '音质', icon: 'wave-sine',
    cell: (t, all) => {
