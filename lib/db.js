...
// 清空包含指定通道标签的重复组（纯组和混合组都删）。
// 仅保留完全不属于该通道的组，由后续匹配步骤与新房结果重新计算并集。
export function clearGroupsByLane(db, laneTagKeys) {
  const targetTags = new Set(laneTagKeys);
  const groups = db.all('SELECT id, group_tags FROM dup_groups');
  let deleted = 0;
  for (const g of groups) {
    const tags = (g.group_tags || '').split(',').filter(Boolean);
    const methodTags = tags.filter(t => MATCHING_METHOD_KEYS.has(t));
    // 只要组内有任何一个目标通道的标签就删除
    if (methodTags.some(t => targetTags.has(t))) {
