...
export function queryLibraryByTier(db, { search = '', sort = 'title', order = 'asc', page = 1, limit = 100, format = '', libFilter = 'all', scrapeTier = '' } = {}) {
  const safeSort = ['title', 'artist', 'album', 'format', 'size', 'duration', 'album_year'].includes(sort) ? sort : 'title';
  ...
  const rows = filtered.slice(start, start + +limit).map(({ _tier, ...rest }) => rest);
