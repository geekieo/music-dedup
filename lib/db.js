...
export function getAllFilesForMatching(db) {
  return db.all(`
    SELECT f.*, sm.mb_recording_id AS mb_recording_id, sm.confidence AS scrape_confidence,
      sm.match_basis AS scrape_match_basis, sm.source AS scrape_source
    FROM files f
...
export function queryLibraryAllForTier(db, { search='', format='', libFilter='all' }={}) {
  ...
  return db.all(`
    SELECT f.*,
      ...
      sm.track_number AS scraped_track_number, sm.genre AS scraped_genre,
      sm.mb_recording_id, sm.match_basis AS scrape_match_basis,
      sm.source AS scrape_source
    FROM files f
    LEFT JOIN whitelist w ON w.file_id=f.id
    ${join}
    ${where}
...
function scrapedShapeFromRow(f) {
  return {
...
// scrapeTier: 'green'|'yellow'|'blue'|'red'|'none' (no usable scraped data) | '' (no filter, still needed when sort==='scrape_tier')
export function queryLibraryByTier(db, { search = '', sort = 'title', order = 'asc', page = 1, limit = 100, format = '', libFilter = 'all', scrapeTier = '', ignoreScript = true } = {}) {
  ...
  const withTier = allRows.map(f => ({ ...f, _tier: computeScrapeTier(f, scrapedShapeFromRow(f), ignoreScript) }));
  ...
  // F9: keep the computed tier on each row (as scrape_tier) instead of
  // stripping it — the library row's 刮削 icon reads this directly now
  // instead of recomputing tier client-side (see queryLibrary below for the
  // non-tier-filtered path, and lib/tier.js's header comment for why having
  // two implementations was a bug in the first place).
...
export function queryLibrary(db, { search='', sort='title', order='asc', page=1, limit=100, format='', libFilter='all', ignoreScript=true }={}) {
  ...
  const rows = db.all(`
    SELECT f.*,
      ...
      sm.album AS scraped_album, sm.album_year AS scraped_album_year,
      sm.track_number AS scraped_track_number, sm.genre AS scraped_genre,
      sm.mb_recording_id, sm.match_basis AS scrape_match_basis, sm.source AS scrape_source
    FROM files f
    LEFT JOIN whitelist w ON w.file_id=f.id
    ${join}
    ${where}
  ...
  // F9: compute scrape_tier server-side for just this (small, already-
  // paginated) page — cheap, and means the row's 刮削 icon reads the exact
  // same tier the 刮削分类 filter would compute for it, instead of a
  // separate (and previously incomplete) client-side recomputation.
  for (const f of rows) f.scrape_tier = computeScrapeTier(f, scrapedShapeFromRow(f), ignoreScript);
  return { total, page:+page, limit:+limit, rows };
}
...
export function upsertScrapedMeta(db, m) {
  db.run(`INSERT OR REPLACE INTO scraped_meta
    (file_id,source,mb_recording_id,mb_release_id,title,artist,album,album_year,track_number,genre,confidence,match_basis,scraped_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [m.file_id,m.source,m.mb_recording_id||null,m.mb_release_id||null,
     m.title,m.artist,m.album,m.album_year||0,m.track_number||0,m.genre||null,m.confidence||0,
...
export function getFilesNeedingScrape(db, { smartScan=true, retryMissed=false } = {}) {
  ...
  // Default: only files never scraped at all ("新增曲目"). retryMissed also
  // includes files that WERE scraped but came back empty (source='none') —
  // for the 高级 → "未命中重新执行" button, not the default smart run (which
  // would otherwise silently re-attempt the same probably-still-unmatched
  // files on every single smart scan).
  const cond = retryMissed ? "(sm.file_id IS NULL OR sm.source='none')" : "sm.file_id IS NULL";
  return db.all(`SELECT f.* FROM files f
    LEFT JOIN scraped_meta sm ON sm.file_id=f.id
    WHERE f.title IS NOT NULL AND f.title!='' AND ${cond}
    ORDER BY f.id`);
}

// AcoustID phase: files that HAVE a fingerprint but have NOT been scraped via AcoustID yet.
// F9: this used to always retry any file that wasn't ALREADY specifically
// AcoustID-sourced — meaning every "智能执行"/"刮削匹配-智能执行" run would
// re-attempt AcoustID for every MB-only-scraped file in the whole library,
// every single time, burning through AcoustID's rate limit repeatedly
// instead of only on new tracks (inconsistent with how MB's own smart scan
// behaves). Default now matches MB: only files never scraped by ANYTHING
// yet. retryMissed (高级 → "未命中重新执行") explicitly does the broader
// "give AcoustID a shot at everything it hasn't confirmed yet" pass.
export function getFilesNeedingAcoustidScrape(db, { retryMissed=false } = {}) {
  const cond = retryMissed
    ? "(sm.file_id IS NULL OR sm.source != 'acoustid' OR sm.source='none')"
    : "sm.file_id IS NULL";
  return db.all(`SELECT f.* FROM files f
    LEFT JOIN scraped_meta sm ON sm.file_id=f.id
    WHERE f.chromaprint IS NOT NULL        -- must have fpcalc-generated Chromaprint fingerprint
