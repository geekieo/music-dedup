...
function scrapedShapeFromRow(f) {
  ...
  // then pick the one with the better tier. On tie, prefer AcoustID.
  ...
  // Compute tier for each — the one with the better tier wins. On tie,
  // AcoustID (audio-verified) is preferred.
  const aidTier = computeScrapeTier(f, aidShape, true);
  const mbTier  = computeScrapeTier(f, mbShape, true);
  const rank = t => t === 'green' ? 0 : t === 'blue' ? 1 : t === 'red' ? 2 : 3;
  return (rank(aidTier) <= rank(mbTier)) ? aidShape : mbShape;
}
