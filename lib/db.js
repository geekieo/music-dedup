...
export function insertGroup(db, { similarity, type, created_time }) {
  db.run('INSERT INTO dup_groups (similarity,type,created_time) VALUES (?,?,?)', [similarity, type, created_time]);
...
export function getGroups(db, { resolved, limit=300, offset=0 }={}) {
  ...
  return db.all(`
    SELECT g.*, COUNT(gt.file_id) AS track_count,
...
export function getWhitelist(db) {
  return db.all('SELECT w.*,f.path,f.title,f.artist,f.album FROM whitelist w JOIN files f ON f.id=w.file_id');
}
