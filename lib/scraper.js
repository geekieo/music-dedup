...
export async function runScrape(db, { smartScan=true, retryMissed=false, acoustidKey='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  // ── Phase A: AcoustID ────────────────────────────────────────────────────
  // Runs independently from MB — by default only processes files never
  // scraped by anything yet ("新增曲目", same restraint as MB's own smart
  // scan); retryMissed (高级 → "未命中重新执行") also retries files MB
  // already touched but AcoustID hasn't confirmed, or that came back empty.
  if (acoustidKey) {
    const aidFiles = getFilesNeedingAcoustidScrape(db, { retryMissed });
    ...
    if (aidTotal === 0) {
      ...
      const lastFailReasons=[]; // last few reasons verbatim, for the abort message
