...
export async function scrapeSingleFile(db, fileId, acoustidKey='') {
  ...
  if (acoustidKey && f.chromaprint) {
    meta = await acoustidLookup(acoustidKey, f.chromaprint, f.fingerprint_duration||f.duration, f);
    if (!meta) await sleep(300);
  ...
  if (acoustidKey) {
    ...
    if (aidTotal === 0) {
      ...
      for (const f of aidFiles) {
        ...
        const meta = await acoustidLookup(acoustidKey, f.chromaprint, f.fingerprint_duration||f.duration, f);
        if (meta) {
          upsertScrapedMeta(db, { ...meta, file_id: f.id });
          aidHit++;
          if (meta.match_basis === 'exact') totalExact++;
          totalMatched++;
        }
        aidDone++;
        await sleep(250); // AcoustID: stay well under rate limits
      ...
      onProgress({ phase:'scrape', pct:50, message:`AcoustID 完成：${aidTotal} 个，命中 ${aidHit} 个` });
    }
  }
