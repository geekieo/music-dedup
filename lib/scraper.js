...
async function acoustidLookup(apiKey, fingerprint, duration, f) {
  ...
  try {
    ...
    // Always parse JSON body — AcoustID returns structured errors on non-200
    // status codes (e.g. invalid key), not opaque HTTP errors.
    let data;
    ...
    if (data.status !== 'ok') {
      // Code 4 = invalid API key — abort the entire batch on this error.
    ...
    // Gather releases from both rec.releases and rec.releasegroups[].releases
    // — AcoustID may nest them under releasegroups depending on server version.
...
export async function scrapeSingleFile(db, fileId, acoustidKey='') {
  ...
  // Always try both sources independently. Each writes its own row
  // thanks to the (file_id, source) composite primary key.
  ...
  if (acoustidKey && f.chromaprint) {
    ...
    if (match) {
      ...
      if (reason && reason !== 'no-results' && reason !== 'low-score')
        console.warn(`[AcoustID] 单文件刮削未命中 (file ${f.id}): ${reason}`);
...
export async function runScrape(db, { smartScan=true, retryMissed=false, acoustidKey='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  if (acoustidKey) {
    ...
    if (aidTotal === 0) {
      ...
      onProgress({ phase:'scrape', pct:0, message:`AcoustID 刮削: 无需处理（${parts.join('，')}）` });
    } else {
      const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
      onProgress({ phase:'scrape', pct:0, message:`开始 AcoustID 刮削（${modeLabel}，${aidTotal} 个文件）...` });
      ...
      if (!invalidKeyStop && consecutiveFailures<30) {
        ...
        onProgress({ phase:'scrape', pct:50, message:`AcoustID 刮削完成：${parts.join('，')}` });
      }
    }
  }
  ...
  if (mbTotal === 0) {
    ...
    const msg = `MusicBrainz 刮削: 无需处理（${parts.join('，')}）`;
    onProgress({ phase:'scrape', pct:100, message: msg });
    return totalMatched;
  } else {
    const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
    const pctBase = acoustidKey ? 50 : 0;
    onProgress({ phase:'scrape', pct: pctBase, message:`开始 MusicBrainz 刮削（${modeLabel}，${mbTotal} 个文件）...` });
    ...
    onProgress({ phase:'done', pct:100, message:'刮削完成' });
  }
  return totalMatched;
}
