...
export async function runScrape(db, { smartScan=true, retryMissed=false, acoustidKey='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  async function runAcoustidPhase() {
    ...
    const aidLog = createPhaseLog(prog, { phase: 'scrape', label: 'AcoustID 刮削' });
    if (aidTotal === 0) {
      ...
      aidLog.start(0, `无需处理：${parts.join('，')}`);
      return { hit: 0, total: 0 };
    }
    const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
    aidLog.start(aidTotal, `${modeLabel}，${aidTotal} 个文件`);
    let aidHit=0;
    ...
    for (const f of aidFiles) {
      ...
      if (!f.chromaprint) {
        ...
        live.aidDone++;
        aidLog.tick(live.aidDone, overallPct());
        continue;
      }
    ...
    if (!invalidKeyStop && consecutiveFailures<30) {
      ...
      aidLog.done(`AcoustID 刮削完成：${parts.join('，')}`, { phase: 'scrape', pct: overallPct(), level: 'ok' });
    }
  ...
  async function runMbPhase() {
    const mbLog = createPhaseLog(prog, { phase: 'scrape', label: 'MusicBrainz 刮削' });
    if (mbTotal === 0) {
      ...
      mbLog.start(0, `无需处理：${parts.join('，')}`);
      return { total: 0, matched: 0, exact: 0, failed: 0, empty: true };
    }
    const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
    mbLog.start(mbTotal, `${modeLabel}，${mbTotal} 个文件`);
    ...
    for (const f of mbFiles) {
      ...
      live.mbDone++;
      mbLog.tick(live.mbDone, overallPct());
    }
