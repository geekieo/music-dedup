...
export async function runScrape(db, { smartScan=true, retryMissed=false, acoustidKey='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  const prog = new Progress(onProgress, 'scrape');
  ...
  if (acoustidKey) {
    ...
    if (aidTotal === 0) {
      ...
      prog.subSkip(0, 'AcoustID 刮削', parts.join('，'));
    } else {
      const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
      prog.subBegin(0, 'AcoustID 刮削', `${modeLabel}，${aidTotal} 个文件`);
      ...
      for (const f of aidFiles) {
        ...
        if (invalidKeyStop || consecutiveFailures>=30) {
          prog.say(Math.round(aidDone/aidTotal*50),
            invalidKeyStop
        ...
        if (aidDone % 20 === 0 || aidDone === aidTotal) {
          prog.subProgress(Math.round(aidDone/aidTotal*50), 'AcoustID 刮削',
            `${aidDone} / ${aidTotal}，命中 ${aidHit} 个`);
        }
      }
      if (!invalidKeyStop && consecutiveFailures<30) {
        ...
        prog.subDone(50, 'AcoustID 刮削', parts.join('，'));
      }
    }
  }
  ...
  if (mbTotal === 0) {
    ...
    prog.subSkip(100, 'MusicBrainz 刮削', parts.join('，'));
    prog.done('刮削完成');
    return totalMatched;
  } else {
    const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
    const pctBase = acoustidKey ? 50 : 0;
    prog.subBegin(pctBase, 'MusicBrainz 刮削', `${modeLabel}，${mbTotal} 个文件`);
    ...
    for (const f of mbFiles) {
      ...
      if (mbDone % 10 === 0 || mbDone === mbTotal) {
        ...
        prog.subProgress(pctBase2 + Math.round(mbDone/mbTotal*(100-pctBase2)), 'MusicBrainz 刮削',
          `${mbDone} / ${mbTotal}（精确 ${mbExact}，模糊 ${mbMatched-mbExact}，未找到 ${mbFailed}）`,
    ...
    const aidSummary = acoustidKey
      ...
      : '';
    prog.say(100,
      `刮削完成：MB ${mbTotal} 个文件，精确 ${mbExact} 个，模糊 ${mbMatched-mbExact} 个，未找到 ${mbFailed} 个${aidSummary}`,
