...
// 网络层可注入：主进程注入 Electron net.fetch（Chromium 栈，跑在 IO 线程、
...
// 无论底层是哪个 fetch 都统一套超时：MusicBrainz/AcoustID 在部分网络下连接会无限挂起，
// 无超时会让扫描刮削阶段死等（worker 内 globalThis.fetch 无默认超时，net.fetch 也无）。
let httpFetchImpl = globalThis.fetch;
export function setHttpFetch(fn) { if (typeof fn === 'function') httpFetchImpl = fn; }
const FETCH_TIMEOUT = parseInt(process.env.SCRAPE_TIMEOUT_MS || '15000', 10);
async function httpFetch(url, opts = {}) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), FETCH_TIMEOUT);
  try { return await httpFetchImpl(url, { ...opts, signal: ctrl.signal }); }
  catch (e) {
    if (e?.name === 'AbortError') throw new Error(`刮削请求超时（${Math.round(FETCH_TIMEOUT / 1000)}s）`);
    throw e;
...
async function mbMatch(f, { withCandidates = false } = {}) {
  const candidates = await mbSearchCandidates(f.title, f.artist);
  ...
  // 手动刮削需要候选列表：复用同一次搜索的结果打分，不再发起第二次 MB 请求。
  if (withCandidates) {
    const scored = candidates.map(rec => scoreCandidate(f, rec));
    scored.sort((a, b) => b.score - a.score);
    return { meta, candidates: scored };
  }
  return meta;
}
...
async function acoustidLookup(apiKey, fingerprint, duration, f) {
  ...
  if (error) return { match: null, reason: error, candidates };
  return { match: candidates[0].candidate, reason: null, candidates };
}
...
// 双源均启用时并行发起两个请求，重叠各自网络延迟；两源限速独立（不同 API），
// 落库写 (file_id, source) 不同行（单线程事件循环内同步串行），无冲突。
// withCandidates：手动刮削时把两次搜索的完整候选列表一并返回（复用同一次请求的
// 结果，不为候选列表再发第二次请求）；批量扫描不传，省去评分开销。
async function scrapeOneFile(db, f, { acoustidKey='', skipMb=false, withCandidates=false }={}) {
  ...
  if (acoustidKey && f.chromaprint) {
    tasks.push((async () => {
      const { match, reason, candidates } = await acoustidLookup(acoustidKey, f.chromaprint, f.fingerprint_duration||f.duration, f);
      result.aid = { match, reason };
      if (withCandidates) result.aidCandidates = candidates || [];
      if (match) {
  ...
  if (!skipMb) {
    tasks.push((async () => {
      const mbRes = await mbMatch(f, { withCandidates });
      const mbMeta = withCandidates ? mbRes.meta : mbRes;
      result.mb = mbMeta;
      if (withCandidates) result.mbCandidates = mbRes.candidates || [];
      if (mbMeta) {
...
// 返回 { mb, acoustid, mbCandidates, aidCandidates, aidError }：候选列表复用本次
// 刮削的搜索结果（见 scrapeOneFile withCandidates），UI 无需再单独拉候选接口。
export async function scrapeSingleFile(db, fileId, acoustidKey='', { skipMb=false }={}) {
  ...
  if (!f) return null;
  const r = await scrapeOneFile(db, f, { acoustidKey, skipMb, withCandidates: true });
  // AID 未执行（未配置 key / 无声纹）时给出与 /acoustid-candidates 一致的空结果原因，
  // 前端据此显示对应提示（"未配置 API Key" 而非笼统的 "未找到匹配结果"）。
  const aidError = r.aid ? r.aid.reason : (!acoustidKey || !f.chromaprint ? 'no-key-or-fingerprint' : null);
  return { ...getScrapedMeta(db, fileId),
    mbCandidates: r.mbCandidates || [],
    aidCandidates: r.aidCandidates || [],
    aidError };
}
...
export async function runScrape(db, { smartScan=true, retryMissed=false, acoustidKey='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  // 两个源各自的文件集与时间占比（AcoustID ~400ms/个，MB ~1100ms/个）：占比用于
  // 双源并行时把两阶段进度按耗时权重合并成整体进度，MB 段不用再以 AID 完成作前提。
  const mbFiles = getFilesNeedingScrape(db, { smartScan, retryMissed });
  const mbTotal = mbFiles.length;
  const aidFiles = acoustidKey ? getFilesNeedingAcoustidScrape(db, { smartScan, retryMissed }) : [];
  const aidTotal = aidFiles.length;
  const aidCost = aidTotal * 0.4, mbCost = mbTotal * 1.1;
  const aidSplit = aidCost + mbCost > 0 ? Math.round(aidCost / (aidCost + mbCost) * 100) : 0;

  // 双源并行共享进度：两阶段各自完成比例按时间占比加权合并，单调不减（进度条不回跳）
  const live = { aidDone: 0, aidTotal, mbDone: 0, mbTotal };
  const overallPct = () => {
    const a = live.aidTotal ? live.aidDone / live.aidTotal : 0;
    const m = live.mbTotal ? live.mbDone / live.mbTotal : 0;
    return Math.round(a * aidSplit + m * (100 - aidSplit));
  };
  const innerPct = (done, total) => Math.round((total ? done / total : 1) * 100);

  // ── Phase A: AcoustID ────────────────────────────────────────────────────
  async function runAcoustidPhase() {
    if (!acoustidKey) return null;
    if (aidTotal === 0) {
      ...
      prog.emit({ phase:'scrape', pct:overallPct(), level:'info', message:`AcoustID 刮削: 无需处理（${parts.join('，')}）` });
      return { hit: 0, total: 0 };
    }
    const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
    prog.emit({ phase:'scrape', pct:overallPct(), subPct:0, level:'info', message:`开始AcoustID 刮削（${modeLabel}，${aidTotal} 个文件）...` });
    let aidHit=0;
    ...
    for (const f of aidFiles) {
      ...
      if (invalidKeyStop || consecutiveFailures>=30) {
        prog.emit({ phase:'scrape', pct:overallPct(), subPct:innerPct(live.aidDone, aidTotal), level:'err',
      ...
      if (live.aidDone % 5 === 0 || live.aidDone === aidTotal) {
        prog.emit({ phase:'scrape', pct:overallPct(), subPct:innerPct(live.aidDone, aidTotal), level:'ok',
          message:`AcoustID 刮削: ${live.aidDone} / ${aidTotal}，命中 ${aidHit} 个` });
      }
    }
    if (!invalidKeyStop && consecutiveFailures<30) {
      ...
      prog.emit({ phase:'scrape', pct:overallPct(), subPct:100, level:'ok', message:`AcoustID 刮削完成：${parts.join('，')}` });
    }
    return { hit: aidHit, total: aidTotal };
  }

  // ── Phase B: MusicBrainz ─────────────────────────────────────────────────
  async function runMbPhase() {
    if (mbTotal === 0) {
      ...
      prog.emit({ phase:'scrape', pct:overallPct(), subPct:100, level:'info', message:`MusicBrainz 刮削: 无需处理（${parts.join('，')}）` });
      return { total: 0, matched: 0, exact: 0, failed: 0, empty: true };
    }
    const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
    prog.emit({ phase:'scrape', pct:overallPct(), subPct:0, level:'info', message:`开始MusicBrainz 刮削（${modeLabel}，${mbTotal} 个文件）...` });

    let mbMatched=0, mbExact=0, mbFailed=0;
    for (const f of mbFiles) {
      ...
      live.mbDone++;
      if (live.mbDone % 5 === 0 || live.mbDone === mbTotal) {
        prog.emit({ phase:'scrape', pct:overallPct(), subPct:innerPct(live.mbDone, mbTotal), level:'ok',
          message:`MusicBrainz 刮削: ${live.mbDone} / ${mbTotal}（精确 ${mbExact}，模糊 ${mbMatched-mbExact}，未找到 ${mbFailed}）`,
          filesProcessed: live.mbDone });
      }
    }
    return { total: mbTotal, matched: mbMatched, exact: mbExact, failed: mbFailed, empty: false };
  }

  // ── 调度：双源（AcoustID + MB）并行，两源各自限速互不干扰，重叠网络延迟省整体
  // 运行时间；仅 MB 单源时保持原顺序行为。
  if (acoustidKey) {
    const [, mbRes] = await Promise.all([runAcoustidPhase(), runMbPhase()]);
    if (mbRes.empty) { prog.done('刮削完成'); return totalMatched; }
    const aidSummary = ` | AcoustID ${(() => {
    ...
    prog.emit({ phase:'scrape', pct:100, level:'ok',
      message:`刮削完成：MB ${mbRes.total} 个文件，精确 ${mbRes.exact} 个，模糊 ${mbRes.matched-mbRes.exact} 个，未找到 ${mbRes.failed} 个${aidSummary}`,
      matched: totalMatched, exact: totalExact, failed: mbRes.failed });
    prog.done('刮削完成');
  } else {
    const mbRes = await runMbPhase();
    if (mbRes.empty) { prog.done('刮削完成'); return totalMatched; }
    prog.emit({ phase:'scrape', pct:100, level:'ok',
      message:`刮削完成：MB ${mbRes.total} 个文件，精确 ${mbRes.exact} 个，模糊 ${mbRes.matched-mbRes.exact} 个，未找到 ${mbRes.failed} 个`,
      matched: totalMatched, exact: totalExact, failed: mbRes.failed });
    prog.done('刮削完成');
  }
  return totalMatched;
