...
import { upsertScrapedMeta, getFilesNeedingScrape, getFilesNeedingAcoustidScrape, updateFileAcoustidChecked, updateFileMbChecked } from './db.js';
...
function selectMatch(f, candidates) {
  ...
  if (!ambiguous && top.score >= 80) {
    ...
    // F9 bugfix: this used to call it "exact" whenever the file had EITHER
    // an album tag OR a duration — but virtually every real file has a
    // duration, so "模糊匹配" was practically unreachable through this
    // branch regardless of how weak the actual corroboration was (this is
    // what was behind "MB 刮削全是精确匹配，一个模糊匹配都没有"). "Exact"
    // should mean real corroboration: either the album text itself lines
    // up well, or the duration matches tightly AND the title-search score
    // was very high. A bare top-hit with no album and just "some" duration
    // agreement is a text-relevance guess — that's 模糊匹配, not 精确匹配.
  ...
  if (hasRichMeta) {
    ...
    if (best && bestScore >= 0.55 && (bestAlbumSim >= 0.45 || (bestDur ?? 0) >= 0.6)) {
      // F9 bugfix: same principle as above — album corroboration is strong
      // evidence (exact), duration-only corroboration on an ambiguous
      // result is weaker (fuzzy), not automatically "exact".
...
// F9: MusicBrainz's /recording search endpoint embeds each candidate
// recording's `releases` array, but that list isn't guaranteed complete —
// for a track re-released many times (reissues, greatest-hits, remasters),
// the search index can omit the earliest/original release entirely, which
// made pickRelease() correctly pick "earliest of what we got" while still
// landing on e.g. a 2021 reissue instead of the true 2008 original. This
// fetches the recording's FULL release list directly (not truncated by
// search relevance) so the earliest-year pick is accurate.
...
async function mbMatch(f) {
  ...
  // Only worth the extra request when we have an album tag to pick a
  // release against, and only for a confirmed (not fuzzy-guessed) match —
  // this roughly doubles request volume for the subset of files where it
  // actually affects the picked release, not the whole library.
  if (meta && f.album && meta.match_basis === 'exact' && meta.mb_recording_id) {
    await sleep(1100); // MusicBrainz: max 1 req/sec — this is a 2nd request for this file
...
//
// F9 bugfix #1: this used to swallow EVERY failure (HTTP error, rate limit,
...
// acoustidLookup now returns a small `{ match, reason }` so the caller can
// aggregate + surface reasons instead of just counting misses.
//
// F9 bugfix #2: switched from GET (fingerprint + all params in the query
// string) to POST (form-encoded body). A real Chromaprint fingerprint for a
// multi-minute track is commonly several thousand characters — long enough
// that some proxies/load balancers in front of api.acoustid.org reject the
// resulting GET URL outright with a plain HTTP 400 that carries no useful
// AcoustID-level error body, which is exactly the "http-400 on every single
// file" symptom this was causing. AcoustID's web service explicitly documents
// request-body handling for POST, so POST is the supported, robust way to
// send a lookup regardless of fingerprint length.
async function acoustidLookup(apiKey, fingerprint, duration, f) {
  ...
  if (roundedDuration <= 0) return { match: null, reason: 'no-duration' }; // AcoustID requires a positive duration
  try {
    const body = new URLSearchParams({
      client: apiKey, duration: String(roundedDuration),
      // F9: was 'recordings releases' — missing releasegroups (release
      // type/grouping data) and tracks (track position within the
      // release), which is why AcoustID hits came back noticeably thinner
      // than MusicBrainz hits (年份/曲目号 often blank). Note genre still
      // won't show up even with this — AcoustID's lookup API doesn't
      // expose genre/tag data at all, only MusicBrainz's own webservice does.
    ...
    // F9 bugfix: this used to bail out on `!res.ok` WITHOUT ever reading the
    // body — but 设置页 Key 验证 (which works: 用户已确认验证通过) parses
    // the JSON body regardless of HTTP status and only cares about the
    // status/error.code fields inside it. AcoustID can return a perfectly
    // structured JSON error (e.g. code 3 invalid key, or a validation
    // error) on a non-200 status, which this code was silently treating as
    // an opaque "http-400" instead of reading what it actually said —
    // exactly how a real, diagnosable error stayed invisible. Now: always
    // try to parse JSON first; only fall back to a bare http-NNN reason if
    // parsing itself fails (i.e. the response genuinely wasn't structured,
    // e.g. an upstream proxy/HTML error page).
    let data;
    ...
    if (data.status !== 'ok') {
      // F9 bugfix: confirmed against a real invalid key that AcoustID's
      // "invalid API key" error is code 4 with message "invalid API key" —
      // NOT code 3 as previously assumed. Matching the wrong code meant an
      // actually-invalid key never hit the fast-abort path below and just
      // ground through the entire library failing identically on every
      // file. Match on the message text as well so this doesn't silently
      // break again if the exact code number is ever misremembered.
...
export async function runScrape(db, { smartScan=true, retryMissed=false, acoustidKey='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  // ── Phase A: AcoustID ────────────────────────────────────────────────────
  // 独立于 MusicBrainz 刮削的可选通道。使用 `files.acoustid_checked_at` 跟踪
  // 自己的处理状态，不依赖 `scraped_meta` 表，确保 AcoustID 和 MB 互不干扰。
  if (acoustidKey) {
    const aidFiles = getFilesNeedingAcoustidScrape(db, { smartScan, retryMissed });
    const aidTotal = aidFiles.length;

    // Diagnostic summary based on AcoustID's OWN tracking
    ...
    if (aidTotal === 0) {
      ...
      if (totalWithChroma === 0) {
        parts.length = 0;
        parts.push('没有文件具备 Chromaprint 指纹（需安装 fpcalc 并运行声纹提取）');
      } else if (retryMissed) {
      ...
      onProgress({ phase:'scrape', pct:0, message:`AcoustID: 无需处理（${parts.join('，')}）` });
    } else {
      const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
      onProgress({ phase:'scrape', pct:0, message:`AcoustID 阶段（${modeLabel}）：共 ${totalWithChroma} 个有声纹文件，本次处理 ${aidTotal} 个...` });
      ...
      for (const f of aidFiles) {
        ...
        if (match) {
          ...
          // Only mark as checked for definitive results. Transient errors
          // (http-*, network-error, rate-limited) should be retried on the
          // next smart scan; no-recording / no-results / low-score are
          // definitive — the fingerprint won't gain a recording later.
      ...
      if (!invalidKeyStop && consecutiveFailures<30) {
        ...
        // Transient: should be retried (not marked as checked)
        ...
        onProgress({ phase:'scrape', pct:50, message:`AcoustID 完成：${parts.join('，')}` });
      }
    }
  }

  // ── Phase B: MusicBrainz ─────────────────────────────────────────────────
  // 独立于 AcoustID 的默认刮削通道。使用 `files.mb_checked_at` 跟踪自己的状态。
  // 写入 scraped_meta 前检查 AcoustID 是否已确认，避免覆盖指纹验证结果。
  const mbFiles = getFilesNeedingScrape(db, { smartScan, retryMissed });
  const mbTotal = mbFiles.length;

  // Diagnostic summary based on MB's OWN tracking
  ...
  if (mbTotal === 0) {
    ...
    const msg = `MusicBrainz: 无需处理（${parts.join('，')}）`;
    onProgress({ phase:'scrape', pct:100, message: msg });
    return totalMatched;
  } else {
    const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
    const pctBase = acoustidKey ? 50 : 0;
    onProgress({ phase:'scrape', pct: pctBase, message:`MusicBrainz 阶段（${modeLabel}）：共 ${totalWithTitle} 个有标题文件，本次处理 ${mbTotal} 个...` });
    ...
    for (const f of mbFiles) {
      ...
      if (meta) {
        // Don't overwrite AcoustID fingerprint-verified results
        const existingAid = db.get("SELECT 1 FROM scraped_meta WHERE file_id=? AND source='acoustid'", [f.id]);
        if (!existingAid) {
          upsertScrapedMeta(db, { ...meta, file_id: f.id });
        }
        ...
        // Write source='none' only if AcoustID hasn't confirmed this file
        const existingAid = db.get("SELECT 1 FROM scraped_meta WHERE file_id=? AND source='acoustid'", [f.id]);
        if (!existingAid) {
      ...
      if (mbDone % 10 === 0 || mbDone === mbTotal) {
        ...
        onProgress({ phase:'scrape', pct: pctBase2 + Math.round(mbDone/mbTotal*(100-pctBase2)),
