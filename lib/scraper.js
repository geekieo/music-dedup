...
export async function runScrape(db, { smartScan=true, retryMissed=false, acoustidKey='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  if (acoustidKey) {
    ...
    if (aidTotal === 0) {
      ...
      prog.emit({ phase:'scrape', pct:0, level:'info', message:`AcoustID 刮削: 无需处理（${parts.join('，')}）` });
    } else {
  ...
  if (mbTotal === 0) {
    ...
    prog.emit({ phase:'scrape', pct:100, subPct:100, level:'info', message:`MusicBrainz 刮削: 无需处理（${parts.join('，')}）` });
    prog.done('刮削完成');
    return totalMatched;
  } else {
    const modeLabel = !smartScan ? '强制重扫' : retryMissed ? '未命中重试' : '智能模式';
