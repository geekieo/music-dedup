...
function selectMatch(f, candidates) {
  ...
  // derived title) — fall back to the top-hit heuristic.
  if (top.score >= 70) {
...
async function mbMatch(f, { withCandidates = false } = {}) {
  ...
  // 手动刮削需要候选列表：复用同一次搜索的结果打分（不另发 MB 请求）。
  if (withCandidates) {
    const scored = candidates.map(rec => scoreCandidate(f, rec));
    scored.sort((a, b) => b.score - a.score);
...
export async function runScrape(db, { smartScan=true, retryMissed=false, acoustidKey='', onProgress=()=>{}, onAbort=()=>false, onPause=async()=>{} }={}) {
  ...
  // 双源并行时把两阶段进度按各源耗时占比合并成整体进度；两源独立调度，互不等待。
  ...
  async function runAcoustidPhase() {
    ...
    if (aidTotal === 0) {
      ...
      prog.emit({ phase:'scrape', pct:overallPct(), level:'info', message:`开始AcoustID 刮削（无需处理：${parts.join('，')}）...` });
  ...
  async function runMbPhase() {
    if (mbTotal === 0) {
      ...
      prog.emit({ phase:'scrape', pct:overallPct(), subPct:100, level:'info', message:`开始MusicBrainz 刮削（无需处理：${parts.join('，')}）...` });
