// electron/ipc/files.js — 文件域：详情 / reveal（shell）/ in-group / rename / live-tags
...
export const routes = [
  ...
  // 文件是否属于某个重复组（优先未处理组）
  { method: 'GET', path: '/api/files/:id/in-group', handler: (p) => {
    const row = db.get(
    ...
    return { ok: true, inGroup: !!row, groupId: row ? row.group_id : null };
  } },
  ...
  { method: 'POST', path: '/api/files/:id/rename', handler: (p, _q, body) => {
    const { newName } = body;
    if (!newName) return { ok: false, error: 'newName required' };
    return renameFile(db, +p.id, newName);
  } },
  // 读取音频文件本身的真实标签（非数据库）
